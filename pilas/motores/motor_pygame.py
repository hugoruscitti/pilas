...
class PygameActor(pygame.sprite.Sprite):
    ...
    def definir_imagen(self, imagen):
        self.image = imagen
        self.image_original = imagen.convert_alpha()
    ...
    def dibujar(self, aplicacion):
        ...
        # TODO: Hacer un cache, para que la imagen no se transforme siempre.
        imagen = pygame.transform.rotozoom(self.image, -self._rotacion_actual, 
                self._escala_actual)
        nuevo_rect = imagen.get_rect()
        nuevo_rect.center = centro
        x, y = nuevo_rect.topleft

        aplicacion.blit(imagen, (x + 320 - self.centro_x - pilas.motor.camara_x, 
...
class Canvas:
    ...
    def actualizar(self):
        self.image = pygame.image.frombuffer(self.data.tostring(), 
                (self.ancho, self.alto), "RGBA")
        
    def limpiar(self):
        import array
        self.data = array.array('c', chr(0) * self.ancho * self.alto * 4)
        stride = self.ancho * 4
        self.surface = cairo.ImageSurface.create_for_data(self.data, cairo.FORMAT_ARGB32, 
                self.ancho, self.alto, stride)
...
class Grilla:
    ...
    def asignar(self, sprite):
        ...
        sprite._actor.rect.width = cuadro_de_imagen.get_width()
        sprite._actor.rect.height = cuadro_de_imagen.get_height()
